...
namespace AdvancedPeopleSystem
{
  ...
  public class CharacterCustomization : MonoBehaviour
  {
    ...
    private void Start()
    {
      sb = GameObject.Find("GameObject").GetComponent<StartButton>();
      pv = GetComponent<PhotonView>();
      SceneName = SceneManager.GetActiveScene().name;
      if (SceneName == "DemoScene")
      {
        sb.CallGetAvatar(sb.userId);
      }
      else
      {
        sb.CallGetAvatar(pv.Owner.NickName);
      }
        }
    ...
    private void LateUpdate()
    {
      ...
      if (sb.userId.Length > 0 && !isSet)
      {
        if (SceneName == "DemoScene")
        {
          LoadCharacterFromFile(sb.userId);
        }
        else
        {
          LoadCharacterFromFile(pv.Owner.NickName);
        }
      }
    }
...
#region Code to exclude the UnHide parts of the character if are hidden in other presets
    ...
    {
      if (!sb.userAvatars.ContainsKey(path)) return;

      string data = sb.userAvatars[path];
      ...
      if (data.Length > 0)
      {
        ...
        if (setup != null)
        {
          Debug.Log("Load완료 " + path);
          SetCharacterSetup(setup);
          sb.userAvatars.Remove(path);
          isSet = true;
    ...
    public void SaveCharacterToFile(CharacterCustomizationSetup.CharacterFileSaveFormat format, string path = "", string name = "")
    {
      ...
      if (path.Length == 0)
      {
        ...
        var folderPath = string.Format("{0}", dataPath);
        ...
        savepath = string.Format("{0}/{1}.{3}", folderPath, name, DateTimeOffset.Now.ToUnixTimeSeconds(), formatString);

      }
      else
      {
        savepath = string.Format("D:/Download/character/appack25.{3}", path, name, DateTimeOffset.Now.ToUnixTimeSeconds(), formatString);
      }
      var data = GetSetup().Serialize(format);
      if (data.Length > 0)
      {
        sb.CallSaveAvatar(data.Trim());
      }
    }
